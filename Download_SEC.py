...
path = f'./Current_Fund_Data/{fund}.xlsx'
date = '08-2021'
...
def org():
    ...
    return datos
df=org()
path = f'.Old_Fund_Data/{fund}/{fund}.xlsx'
...
def download(df, path):
    try:
        ...
        os.mkdir(f'./Old_Fund_Data/{fund}')
    
download(df, path)

if not os.listdir(f'./Old_Fund_Data/{fund}') :
    try:
        with pd.ExcelWriter(path ,engine='openpyxl', mode='w') as writer:
            df.to_excel(writer, sheet_name=date)
    except FileNotFoundError:
...
print("Done ;)")
print("------------------------------------------------------------------------------")
print(f"The organized data is stored in: {path}")
